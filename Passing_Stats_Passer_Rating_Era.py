...
#########################################################################################
    ...
    def load_data(selected_year):
        ...
        df3.to_csv("NFL_Player_QB_Search_Images.csv")
        return df3
    df3 = load_data(selected_year)
    ...
    # To double check that the DataFrame Merge was Successful
    ...
    if user_input_demo_player_:
        ...
        z = df_merged.loc[df_merged['Player']==user_input_demo_player_, ['Player_Image']]
        for i in z.Player_Image:
            get_url = requests.get(i)
            img = Image.open(BytesIO(get_url.content))
            #img.show()
            # This turns off the axes for the plotted player image
            #plt.axis('off')
            #imgplot = plt.imshow(img)
            
            #plt.savefig(imgplot)
            #st.pyplot(imgplot)
            st.image(img, caption=user_input_demo_player_)
